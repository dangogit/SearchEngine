...
class Indexer:
    ...
    def __init__(self, config):
        ...
        self.inverted_idx_files_list = ["inverted_idx_a_to_c.json", "inverted_idx_d_to_h.json", "inverted_idx_i_to_o.json",
                                   "inverted_idx_p_to_r.json", "inverted_idx_s_to_z.json", "inverted_idx_hashtags.json"]
        self.posting_dicts_list = [{}, {}, {}, {}, {}, {}]
        self.posting_files_list = ["posting_file_a_to_c.json", "posting_file_d_to_h.json", "posting_file_i_to_o.json",
                                   "posting_file_p_to_r.json", "posting_file_s_to_z.json", "posting_file_hashtags.json"]
        self.config = config
        self.key = 0
        self.curr = 0
        self.term_index = 0
    ...
    def update_posting_file(self):
        ...
        for i in range(len(self.inverted_idx_files_list)):
            try:
                with open(self.inverted_idx_files_list[i], 'r', encoding='utf-8') as inverted_idx_file:
                    inverted_idx_from_file = json.load(inverted_idx_file)
            except:
                inverted_idx_from_file = {}

            inverted_idx_to_file = self.merge_inverted_idx_dicts(inverted_idx_from_file, self.inverted_idx_dicts_list[i])
            # to json:
            try:
                with open(self.inverted_idx_files_list[i], 'w', encoding='utf-8') as inverted_idx_file:
        ...
        for i in range(len(self.posting_files_list)):
            try:
                with open(self.posting_files_list[i], 'r', encoding='utf-8') as posting_file:
            ...
            try:
                ...
                with open(self.posting_files_list[i], 'w', encoding='utf-8') as posting_file:
