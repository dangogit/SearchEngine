...
class Indexer:
    ...
    def add_new_doc(self, document):
        ...
        for term in document_dictionary.keys():
            try:
                ...
                self.insert_term_to_inv_idx_and_post_dict(term, freq_in_doc, doc_idx, unique_terms_in_doc, max_tf,
                                                          document)
    ...
    def insert_term_to_inv_idx_and_post_dict(self, term, freq_in_doc, doc_idx,
        ...
        self.inverted_idx_dicts_list[index][term] = [number_of_docs, self.differnce_method(docs_list, last_doc_idx),
                                                     doc_idx]
        key = term + " " + str(doc_idx)
        term_indices = [idx for idx, word in enumerate(document.full_text.split(), 1) if word == term]
        self.posting_dicts_list[index][key] = [doc_idx, freq_in_doc, term_indices, document.doc_length,
                                               unique_terms_in_doc, max_tf]
    ...
    def fix_big_and_small_letters(self, inverted_dict):
        ...
        for term in inverted_dict.keys():
            if term[0] != '#' and term not in self.words_dict.keys():
                new_term = term.upper()
